...
test('vscode1640ProjectItems fallback to menubar storage', async () => {
    ...
    expect(items[3].title).toEqual('menu-workspace(工作区)')
    // backupWorkspaces 次之
    expect(items[4].title).toEqual('backup(工作区)')
    expect(items[5].title).toEqual('project(工作区)')
    expect(items[5].command.command).toContain('--file-uri')
    expect(items[6].title).toEqual('backup')
    expect(items[7].title).toEqual('project')
    expect(items[7].command.command).toContain('--folder-uri')
    // profileAssociations 兜底
    expect(items[8].title).toEqual('profile-workspace(工作区)')
    expect(items[9].title).toEqual('profile-project')
})
